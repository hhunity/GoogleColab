...
int main(int argc, char** argv) {
    ...
    float* d_img1[2] = {nullptr, nullptr};
...
#endif
            CHECK_CUDA(cudaMemcpyAsync(peaks_host[buf], d_final_peaks[buf], static_cast<size_t>(batch) * sizeof(Peak),
                                       cudaMemcpyDeviceToHost, stream));
            CHECK_CUDA(cudaMemcpyAsync(cent_host[buf], d_centroids[buf], static_cast<size_t>(batch) * sizeof(Centroid),
                                       cudaMemcpyDeviceToHost, stream));

            CHECK_CUDA(cudaStreamEndCapture(stream, &graphs[buf]));
            CHECK_CUDA(cudaGraphInstantiate(&graph_exec[buf], graphs[buf], nullptr, nullptr, 0));
            graph_ready[buf] = true;
        }

        // グラフ実行
        CHECK_CUDA(cudaGraphLaunch(graph_exec[buf], stream));
    ...
    for (int b = 0; b < 2; ++b) {
        if (graph_exec[b]) cudaGraphExecDestroy(graph_exec[b]);
