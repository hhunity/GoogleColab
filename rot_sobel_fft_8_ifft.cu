...
int main(int argc, char** argv) {
    ...
    // PFMで読み込む外部スペクトル入力
    std::string pfm_path = (argc >= 5) ? argv[4] : "";
    ...
    if (pfm_in.width != img.width || pfm_in.height != img.height || pfm_in.channels != 1) {
        std::fprintf(stderr, "PFM input size must match and be grayscale\n");
        return 1;
    }
    ...
    float* d_sobel_f = nullptr; // FFT1入力用
    ...
    // FFT2（PFM入力）を1回だけ前計算: PFMはFFT後の画像（実部/虚部）を想定。
    // channels==2または3なら[real, imag, *]、channels==1なら虚部0として扱う。
    {
        std::vector<cufftComplex> fft2_host(fft_elems);
        for (int y = 0; y < fft_h; ++y) {
            for (int x = 0; x < fft_out_cols; ++x) {
                size_t src_idx = static_cast<size_t>(y) * pfm_in.width + x;
                size_t dst_idx = static_cast<size_t>(y) * fft_out_cols + x;
        ...
        CHECK_CUDA(cudaMemcpyAsync(d_fft2, fft2_host.data(), fft_elems * sizeof(cufftComplex),
