...
int main(int argc, char** argv) {
    ...
    // 32bitのままPFMで保存（回転結果とSobel結果）
    std::vector<float> rot_host(img.width * img.height);
    std::vector<float> sobel_host(img.width * img.height);
    CHECK_CUDA(cudaMemcpy(rot_host.data(), d_rot_f, rot_host.size() * sizeof(float), cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(sobel_host.data(), d_mag_f, sobel_host.size() * sizeof(float), cudaMemcpyDeviceToHost));

    PFMImage rot_pfm{img.width, img.height, 1, std::vector<float>(rot_host.begin(), rot_host.end())};
    PFMImage sobel_pfm{img.width, img.height, 1, std::vector<float>(sobel_host.begin(), sobel_host.end())};

    std::string rot_out_path = out_path + "_rot.pfm";
    std::string sobel_out_path = out_path + "_sobel.pfm";
    if (!write_pfm(rot_out_path, rot_pfm)) return 1;
    if (!write_pfm(sobel_out_path, sobel_pfm)) return 1;
    std::printf("Wrote rotated float PFM to %s\n", rot_out_path.c_str());
    std::printf("Wrote Sobel float PFM to %s\n", sobel_out_path.c_str());
