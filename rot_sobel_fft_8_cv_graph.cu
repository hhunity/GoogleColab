...
// float -> float 変換（R2C入力用）＋2Dハン窓を適用してリークを低減
__global__ void u8_to_float_window(const float* src, float* dst, int width, int height) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    if (x >= width || y >= height) return;
...
int main(int argc, char** argv) {
    ...
    cufftSetStream(fft_plan, stream); // graph capture対象のストリーム
    // ホスト側への転送先をピンメモリに確保（graphキャプチャで必要）
    cufftComplex* fft_host_pinned = nullptr;
    CHECK_CUDA(cudaMallocHost(&fft_host_pinned, fft_full_elems * sizeof(cufftComplex)));
    ...
    for (int iter = 0; iter < iters; ++iter) {
        ...
        if (!graph_ready) {
            ...
            CHECK_CUDA(cudaMemcpyAsync(fft_host_pinned, d_fft_full, fft_full_elems * sizeof(cufftComplex),
        ...
        // ピンメモリからstd::vectorへコピー（表示用）これがないと速度はほぼ同じ。
        std::memcpy(fft_host.data(), fft_host_pinned, fft_full_elems * sizeof(cufftComplex));

        auto t_end = std::chrono::steady_clock::now();
        double elapsed_ms = std::chrono::duration<double, std::milli>(t_end - t_start).count();

        total_ms += elapsed_ms;
        std::printf("[iter %d/%d] 128-157行相当の処理時間: %.3f ms\n",
                    iter + 1, iters, elapsed_ms);
    }

    double avg_ms = total_ms / iters;
    std::printf("平均: %.3f ms over %d iters\n", avg_ms, iters);
    ...
    if (fft_host_pinned) cudaFreeHost(fft_host_pinned);
