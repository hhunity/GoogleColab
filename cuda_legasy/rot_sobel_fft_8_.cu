...
// u8 -> float 変換（FFT入力用）＋2Dハン窓を適用してリークを低減
__global__ void u8_to_f32_window(const unsigned char* src, float* dst, int width, int height) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    if (x >= width || y >= height) return;
    ...
    dst[y * width + x] = static_cast<float>(src[y * width + x]) * wx * wy;
}
...
int main(int argc, char** argv) {
    ...
    float* d_sobel_f = nullptr; // FFT入力用にSobel結果をfloatへ変換
    ...
    CHECK_CUDA(cudaMalloc(&d_fft, fft_elems * sizeof(cufftComplex)));
    ...
    for (int iter = 0; iter < iters; ++iter) {
        ...
        // Sobel結果(u8)をFFT入力用にfloatへ変換し、ハン窓を適用してからFFT
        u8_to_f32_window<<<grid2, block2, 0, stream>>>(d_mag, d_sobel_f, img.width, img.height);
        // FFT実行（R2C）
        if (cufftExecR2C(fft_plan, reinterpret_cast<cufftReal*>(d_sobel_f), d_fft) != CUFFT_SUCCESS) {
    ...
    // FFT結果をホストへコピーし、振幅をそのままPFMで保存（正規化なし）。
    std::vector<cufftComplex> fft_host(fft_elems);
    CHECK_CUDA(cudaMemcpy(fft_host.data(), d_fft, fft_elems * sizeof(cufftComplex), cudaMemcpyDeviceToHost));

    // cuFFTのR2Cは横方向が半分なので、対称性を使ってフルサイズの振幅を組み立てる
    std::vector<float> fft_mag_full(img.width * img.height, 0.0f);
    for (int y = 0; y < fft_h; ++y) {
        for (int x = 0; x < img.width; ++x) {
            int src_x = (x < fft_out_cols) ? x : (img.width - x);
            size_t idx = static_cast<size_t>(y) * fft_out_cols + src_x;
            float re = fft_host[idx].x;
            float im = fft_host[idx].y;
            fft_mag_full[y * img.width + x] = std::sqrt(re * re + im * im);
        }
    }
    // 周波数シフト（DCを中央へ）してPFMにそのまま書き出す
    PFMImage fft_pfm;
    fft_pfm.width = img.width;
    fft_pfm.height = img.height;
    fft_pfm.channels = 1;
    fft_pfm.data.resize(static_cast<size_t>(img.width) * img.height);
    int half_w = img.width / 2;
    int half_h = img.height / 2;
    for (int y = 0; y < img.height; ++y) {
        for (int x = 0; x < img.width; ++x) {
            int sx = (x + half_w) % img.width;
            int sy = (y + half_h) % img.height;
            fft_pfm.data[y * img.width + x] = fft_mag_full[sy * img.width + sx];
        }
    }
    std::string fft_out_path = out_path + "_fft.pfm";
    if (!write_pfm(fft_out_path, fft_pfm)) return 1;
    std::printf("Wrote FFT magnitude (PFM, no normalization) to %s\n", fft_out_path.c_str());
